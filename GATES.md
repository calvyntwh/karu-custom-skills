...
- [x] G9: SKILL.md tier numbering per-tier starting at 1 (max number ≤ 10; Tier 1 has 10 items)
...
- [ ] G11: SKILL.md Eval 4 block — 4 content lines (header + Scenario + Expected + Pass criteria)
...
- [ ] G14: Net line change vs base commit ff306db is negative
  CHECK: sh -c 'git diff ff306db..HEAD -- skills/humanizer-karu-custom/ | grep -E "^[+-][^+-]" | awk "/^+/{a++} /^-/{r++} END{print a-r}"'
  EXPECT: -10 (or any negative number; expect net reduction)
  EVIDENCE: pending

- [ ] G15: Installed skill synced from worktree
...
- [ ] G16: Branch pushed with shrink commit
  CHECK: sh -c 'git log origin/humanizer-v5-model-aware --oneline -1 | grep -v ff306db'
  EXPECT: 1 line (a commit hash NOT ff306db)
  EVIDENCE: pending
